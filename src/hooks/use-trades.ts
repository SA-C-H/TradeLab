...
export function useTrades() {
  ...
  return useQuery({
    queryKey: ['trades', session?.user?.id, currentAccount?.id],
...
export type NewTradePayload = Omit<TablesInsert<'trades'>, 'user_id' | 'id' | 'created_at' | 'account_id'>;
...
export function useCreateTrade() {
  ...
  return useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['trades', currentAccount?.id] }),
  });
}
